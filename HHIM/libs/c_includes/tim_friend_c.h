...
#define TIM_UPDATE_FRIEND_GROUP_STATUS_MAX_GROUPS_EXCEED	32521//更新好友分组时有效：该好友加入的好友分组个数超过了限制，每个好友最多只能加入32个好友分组
	...
	typedef void* TIMProfileCustomInfoUintHandle;
	TIM_DECL TIMProfileCustomInfoUintHandle CreateProfileCustomInfoUintHandle();
	TIM_DECL void DestroyProfileCustomInfoUintHandle(TIMProfileCustomInfoUintHandle handle);
	TIM_DECL TIMProfileCustomInfoUintHandle CloneProfileCustomInfoUintHandle(TIMProfileCustomInfoUintHandle handle);
	TIM_DECL bool AddProfileCustomInfoUint(TIMProfileCustomInfoUintHandle handle, const char* key, uint32_t key_len, uint64_t value);
	TIM_DECL uint32_t GetProfileCustomInfoUintNum(TIMProfileCustomInfoUintHandle handle);
	TIM_DECL uint32_t GetProfileCustomInfoUint(TIMProfileCustomInfoUintHandle handle, char* key, uint32_t key_len, uint64_t *value);
	TIM_DECL uint32_t GetProfileCustomInfoUintByIndex(TIMProfileCustomInfoUintHandle handle, int index, char* key, uint32_t* key_len, uint64_t *value);
	...
	TIM_DECL void SetCustomInfoUint4SetProfileOptionHandle(TIMSetProfileOptionHandle handle, TIMProfileCustomInfoUintHandle custom_info);
